...
import LetterPullUp from "../overlays/LetterPullUp";
...
export default function HeroSection() {
  ...
  const techLogos = [
    ...
    {
      name: "Blockchain",
      icon: <SiEthereum className="w-full h-full" />,
      color: "text-[#1f0079]",
    },
    {
      name: "Web App",
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            We build reliable modern technology to empower digital
            transformation. Our engineering-first approach ensures high
            performance, scalability, and long-term impact. We focus on product
            excellence, seamless delivery, and technical precision.
          </motion.p>
        </motion.div>
        ...
        >
          <div className="flex -space-x-2">
            {techLogos
              .slice(0, getResponsiveValue(3, 4, 5))
              .map((tech, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <div className="w-4 h-4 sm:w-5 sm:h-5">{tech.icon}</div>
                </motion.div>
              ))}
          </div>
          <motion.span
      ...
      <div
        className={`relative ${getResponsiveValue(
          "w-full h-64 mt-6 mb-10",
          "w-full h-60 mt-8 mb-12",
          "lg:absolute right-0 w-1/2 h-full "
        )} flex items-center justify-center lg:pr-12 xl:pr-16 `}
        aria-hidden="true"
      >
        {/* Central Object */}
        <div className="relative z-10 ">
          <motion.div
            className={`${getResponsiveValue(
              "w-24 h-24",
              "w-32 h-32",
              "w-40 h-40 lg:w-40 lg:h-40"
            )} rounded-full bg-gradient-to-br from-[#1f0079] to-[#513897] shadow-xl shadow-[#513897]/30 flex items-center justify-center`}
            animate={{
              scale: [1, 1.05, 1],
              rotate: [0, 180, 360],
              boxShadow: [
                "0 0 20px rgba(31, 0, 121, 0.3)",
                "0 0 40px rgba(81, 56, 151, 0.4)",
                "0 0 20px rgba(31, 0, 121, 0.3)",
              ],
            }}
            transition={{
        ...
        <motion.div
          ...
          style={{
            rotate: useTransform(orbitProgress, [0, 1], [0, 360]), // Full 360-degree rotation
          }}
        >
          {techLogos.map((tech, logoIndex) => {
            const distance = getResponsiveValue(100, 125, 125);
            const angle = (logoIndex / techLogos.length) * Math.PI * 2;
            ...
            return (
              <motion.div
                key={tech.name}
                className={`absolute ${tech.color}  bg-white/90 backdrop-blur-sm  rounded-full shadow-lg flex items-center justify-center border border-[#513897]/20`}
                style={{ 
                  x,
                  y,
                  width: `${getResponsiveValue(30, 40, 50)}px`,
                  height: `${getResponsiveValue(30, 40, 50)}px`,
        ...
        {[...Array(getResponsiveValue(10, 20, 30))].map((_, i) => (
          <motion.div
            ...
            style={{
              width: `${
                Math.random() *
                  (screenSize.isMobile ? 2 : screenSize.isTablet ? 2 : 3) +
                1
              }px`, // Smaller particles
              height: `${
                Math.random() *
                  (screenSize.isMobile ? 2 : screenSize.isTablet ? 2 : 3) +
                1
